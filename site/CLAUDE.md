...
- 站外同步：YouTube 影片卡 + Pixiv 圖集 + 巴哈，每月 15 號 content-sync.yml
  自動抓（詳見 docs/SYNC.md）；巴哈備份在上層 repo 的 `archive/`，build 時由
  import-baha.mjs 轉成 `/baha/<sn>/` 文章（比照 blog 的生成模式，覆寫用 baha-overrides.ts）
