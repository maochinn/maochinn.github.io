# [Blender2.8]Python Tests

> 2019-10-05 · Blender · GP 1 · 來源 https://home.gamer.com.tw/artwork.php?sn=4550546

本文已搬運、更新到[medium](https://medium.com/maochinn/blender2-8-testing-76d785c28966)
...
那麼還是先附個[官網連結](https://wiki.blender.org/wiki/Tools/Tests/Python)
...
這邊建了一個marco叫做MAO\_TEST，

第一個參數是module，第二個是測試名稱，

最後一行我給的是mao\_mod跟simple當參數。

  

進到函式，

add\_test，就是增加一個test到ctest裡面。

  

    NAME mao\_test\_${test\_name}

那test的名子就是mao\_test\_${test\_name}

也就是mao\_test\_simple
...
[這邊有詳細可用的參數](https://docs.blender.org/manual/en/latest/advanced/command_line/arguments.html)
...
在這之後是test\_simple.py要用的參數

  

    --testdir=${TEST\_OUT\_DIR}/${module}/output\_${test\_name}.blend

這就是test\_simple.py要用的參數，
...
包含${TEST\_BLENDER\_EXE\_PARAMS}或是${TEST\_SRC\_DIR}.
...
就是如果有寫--write-blend 就會把後面的path給write\_blend，
...
blender-git\\lib\\tests\\mao\_mod\\simple.blend

這邊我就隨便開一個場景建了一顆猴頭![](images/01.png)

5.

要在blender-git\\build\_windows\_Full\_x64\_vc16\_Release\\tests\\
...
比如說沒有add\_subdirectory(mao\_mod)
...
可以去檢查一下blender-git\\build\_windows\_Full\_x64\_vc16\_Release\\tests\\mao\_mod

有沒有多一個檔案是output\_simple.blend
...
接下來要進一步測試就只要一般地去寫那個test\_simple.py的測試腳本就好了!
...
D:\\blender-git\\blender\\tests\\python\\mao\_mod

2.資源路徑

D:\\blender-git\\lib\\tests\\mao\_mod

3.輸出路徑

D:\\blender-git\\build\_windows\_Full\_x64\_vc16\_Release\\tests\\mao\_mod
